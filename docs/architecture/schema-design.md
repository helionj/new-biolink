...
| 2026-05-25 | 0.5     | Story 3.4 materializou bucket `avatars` + 4 RLS policies em `storage.objects` (`supabase/migrations/0006_storage_avatars.sql` + rollback companion). Bucket: `public:true` (1 MB, jpg/png/webp). Policies: `avatars_select_public` (USING bucket_id), `avatars_insert_own`/`avatars_update_own`/`avatars_delete_own` (WITH CHECK/USING `storage.foldername(name)[1] = auth.uid()::text`). Path convention `avatars/{auth.uid()}/<filename>.{ext}`. **DEV-5 (override de L1040 `public:false`)** — `public:true` ratificado pela @data-engineer: justificativas (1) NFR LCP em SSR da página pública `/@<username>` evita round-trip de `createSignedUrl`; (2) RLS preserva owner-boundary em INSERT/UPDATE/DELETE — `public:true` apenas remove signing overhead no SELECT que é público por design (FR13); (3) `profiles.avatar_url` armazena URL estável (signed URL persistida seria anti-pattern). Reversível via `UPDATE storage.buckets SET public=false WHERE id='avatars'` sem recriar URLs. L1040 do §Storage Integration permanece como referência histórica; este Change Log é o registro canônico do override (`schema-design.md` é L2/append-only). Integration test em `tests/integration/rls/storage.test.ts` (5 cenários: anon SELECT, owner INSERT/UPDATE/DELETE, cross-owner INSERT/DELETE block). `profiles.avatar_url` CHECK não alterado (`^https?://` cobre Storage public URL). Trigger `on_auth_user_created` NÃO estendido (avatares não são bootstrapados — user-uploaded). | @data-engineer (Dara) |
| 2026-05-26 | 0.6     | Story 4.2 materializou `page_views` (N:1 com `pages`, `supabase/migrations/0008_page_views.sql` + rollback companion) + 1 índice composto (`idx_page_views_page_id_viewed_at` cobre cascade lookups + AP-6 analytics + dedup window query) + 1 RLS policy (`page_views_select_own`, 1-hop JOIN `pages → profile`). Sem policies INSERT/UPDATE/DELETE (AC4 análogo a 4.1 — service-role bypass). Check constraint `chk_page_views_hash_size` espelhada 100% da 0007. `id bigint identity` coerente com `click_events` (Q2 RESOLVED). Dedup window de 30min implementado app-side em `lib/track.ts insertPageView()` (DEV-2 — Postgres não suporta `now()` em IMMUTABLE predicate; race window <0.1% aceito). DEV-3: `auth.uid()` direto na policy (coerência com 8 policies anteriores; batch fix futuro via `[STORY-4.1-F1]`). Apenas append (doc é L2/extend-only).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | @data-engineer (Dara) |
| 2026-05-27 | 0.7     | Story 4.3 materializou 4 aggregation views (`link_clicks_7d`/`link_clicks_30d` + `page_views_7d`/`page_views_30d`) com `WITH (security_invoker = true)` — herdam RLS de `click_events`/`page_views` automaticamente (PG 15+). 2 helper functions (`get_link_clicks_series` + `get_page_views_series`) com `SECURITY INVOKER STABLE LANGUAGE sql` retornam `TABLE(day date, count bigint)` sparse (dias zerados omitidos — gap-filling é responsabilidade do caller UI). Migration `0009_views_aggregations.sql` + rollback companion. **Sem novos índices** (reuso de `idx_click_events_link_id_clicked_at` 0007 + `idx_page_views_page_id_viewed_at` 0008, ambos cobrem 100% dos predicados de janela `WHERE X >= now() - interval 'N days'`). AC2 estratégia "views regulares para MVP + materialized deferida até P95 dashboard >500ms" satisfeita **por referência** a §4 Materialized Views L655-657 e arch.md §Data Models §Aggregation Views L356-365 (DEV-6 — não duplicar). 25 cenários integration test verdes (9 agregação em `tests/integration/db/aggregations.test.ts` + 16 RLS em `tests/integration/rls/aggregations.test.ts` cobrindo owner/non-owner/anon × 4 views × 2 helpers + validação central security_invoker via comparação admin vs alice). Apenas append (doc é L2/extend-only).                                                                                                                                                                                          | @data-engineer (Dara) |
